// React와 Firebase는 모듈(import) 없이, 일반 <script> 태그로 불러온
// 전역 변수를 사용합니다. 일부 모바일 브라우저가 import map/ES모듈을
// 지원하지 않아서 화면이 안 뜨는 문제가 있었는데, 이 방식이 훨씬 널리 호환돼요.
...
const famDoc = (name, id) => famCol(name).doc(id);
...
function KidsTimetable() {
  ...
  useEffect(() => {
    const unsubItems = famCol("items").onSnapshot((snap) => {
      if (snap.empty) {
    ...
    const unsubTodos = famCol("todos").onSnapshot((snap) => {
      if (snap.empty) {
        const batch = db.batch();
    ...
    const unsubComments = famCol("comments").onSnapshot((snap) => {
  ...
  const writeItem = (item) =>
    famDoc("items", item.id).set(item).catch((e) => console.error("저장 실패", e));
  ...
  const writeTodo = (t) =>
    famDoc("todos", t.id).set(t).catch((e) => console.error("저장 실패", e));
  ...
  const deleteTodo = (id) =>
    famDoc("todos", id).delete().catch((e) => console.error("삭제 실패", e));
  ...
  const writeComment = (c) =>
    famDoc("comments", c.id).set(c).catch((e) => console.error("저장 실패", e));

  const deleteComment = (id) =>
    famDoc("comments", id).delete().catch((e) => console.error("삭제 실패", e));
...
// ---------- 앱 시작 ----------
ReactDOM.createRoot(document.getElementById("root")).render(<KidsTimetable />);
